...
namespace CuaHangLLPro
{
    public static class clsChucNang
    {
        ...
        public static Boolean namcheck(this String s)
        {
            return Regex.Match(s, "((19|20)\\d\\d)").Success;
        }
        
        public static Boolean nameCheck(this String s)
        {
            return Regex.Match(s, "^[a-zA-Z'-'sáàảãạăâắằấầặẵẫậéèẻ ẽẹếềểễệóòỏõọôốồổỗộ ơớờởỡợíìỉĩịđùúủũụưứ�ừ �ửữựÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠ ƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼ� ��ỀỂỄỆỈỊỌỎỐỒỔỖỘỚỜỞ ỠỢỤỨỪỬỮỰỲỴÝỶỸửữựỵ ỷỹ]*$").Success;
        }
        public static Boolean slCheck(this String s)
        {
            return Regex.Match(s, "^[0-9]*$").Success;
        }
        public static Boolean tienChek(this String s)
        {
            return Regex.Match(s, "^([1-9]{1}[0-9]{1,9})$").Success;
        }
